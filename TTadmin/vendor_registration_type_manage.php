...
$vendor_registration_name=$_REQUEST['vendor_registration_name'];

if(isset($_REQUEST['update']))
{

$venreg_id = $_REQUEST['venreg_id'];
$query="update vendor_registration_type SET vendor_registration_name='$vendor_registration_name' WHERE venreg_id=$venreg_id";
...
{
$query="insert into vendor_registration_type(vendor_registration_name) values('$vendor_registration_name')";
mysqli_query($link,$query);
...
$query="update vendor_registration_type SET status='$status' WHERE id='$id'";         
mysqli_query($link,$query);
}
else if(isset($_REQUEST['del']))
{
$venreg_id=$_REQUEST['del'];
$query="delete from vendor_registration_type WHERE venreg_id=$venreg_id";
mysqli_query($link,$query);
}
?>
        ...
        <div class="col-xs-12">
          <div class="box">
            <div class="box-header">
              ...
              <a href="vendor_registration_type_form.php">
...
$sql_cms="select * from vendor_registration_type order by vendor_registration_name ASC"; 
 $result_cms=mysqli_query($link,$sql_cms); 
 while($row_cms=mysqli_fetch_array($result_cms)) { 
?>
                    <tr>
                      <td><input type="checkbox" name="users[]" value="<?php echo $row_cms["venreg_id"]; ?>" /></td>

                      <td><?php 
												
												echo $row_cms['vendor_registration_name'];
												
												?></td>
                     
                      <td><a href="vendor_registration_type_form.php?cms=<?php echo $row_cms['venreg_id'];?>" class="edit_btn"><span>Edit</span></a>
                        
                        <span><a href="vendor_registration_type_manage.php?del=<?php echo $row_cms['venreg_id']; ?>" onClick="return confirm('Do you really want to remove it?')" class="delet"><span>Delete</span></a></span></td>
                    </tr>
